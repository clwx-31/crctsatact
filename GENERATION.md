...
- **Linear equations in two variables:** coordinate substitution, standard-form slope, the x-intercept of a line in slope-intercept form, coefficient interpretation, parallel/perpendicular lines, and the equation of a line through two given points.
...
- **Two-variable data:** model prediction in both directions, reading a value off a scatterplot, slope and intercept interpretation, choosing a line of best fit for plotted data, residuals, the limits of extrapolating beyond the observed range, model selection, recovering an exponential model from a table, the x at which two linear models agree, association, tables, scatterplots, and the gap between a predicted and an observed value.
- **Probability and conditional probability:** simple probability, complements, probability from a frequency table, the union of two disjoint categories, two and three draws without replacement, "at least one" as the complement of "none", conditional and joint probability from two-way tables, independent intersections, the addition rule in both directions, a rate weighted across two unequal subgroups, and expected counts.
...
- **Lines, angles, and triangles:** triangle sums, vertical/corresponding/supplementary angles, an angle chase through the exterior-angle theorem, parallel-line relationships, similarity, and SAS sufficiency.
- **Right triangles and trigonometry:** the Pythagorean theorem for a hypotenuse or a missing leg, right-triangle area, coordinate distance, both special right triangles, identifying and computing sine/cosine/tangent, recovering a side from a given ratio, cofunctions within a triangle, solving sin(ax + b) = cos(cx + d) for x, and unit-circle radians.
- **Circles:** radius/diameter, circumference/area, area from a given circumference, equations and the radius recovered from one, arc length, sector area, an inscribed angle related to the central angle on the same arc, and tangent lines.
...
- In Math, difficulty follows the recipe. A recipe belongs to exactly one tier, and every hard tier draws on at least three recipes so eight hard questions are not eight instances of one mold. A recipe belongs in the hard tier only if it needs more work than that skill's medium recipes; applying a stated formula to stated values is not a hard item however advanced the formula sounds.
